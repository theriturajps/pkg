$(function() {
            ...
            if (id) {
              $.ajax({
                ...
                if (data.webContentLink) {
                  var formattedWebContentLink = data.webContentLink.replace(/&export=download/, '');
                  $('#data').append(`
			<li>
				<a href="${formattedWebContentLink}">${formattedWebContentLink}</a>
			</li>`);
                } else {
                  $('#data').append(`
        ...
        $('#direct-link-form').on('submit', function(e) {
          e.preventDefault();
          idIndex = 0;
          idArray = $('#url').val().split('\n').slice(0, maxLines).join('\n').split(/https:\/\/drive\.google\.com\/open\?id=|https:\/\/drive\.google\.com\/file\/d\//).slice(1);
          // Checks for every id if it has a correct format.
          if (idArray.length > 0 && idArray.every(function(id) {
